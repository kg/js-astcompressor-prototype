...
echo // input
Upstream\7z.exe a -bd -tzip %INFILE%.zip %INFILE% 1>NUL
...
echo // encoded sizes
Upstream\7z.exe a -bd -tzip Test\%FILE_PREFIX%.webasm.zip Test\%FILE_PREFIX%.webasm 1>NUL
...
rem fc /A /N /L Test\%FILE_PREFIX%.ast.json Test\%FILE_PREFIX%.ast.decoded.json
